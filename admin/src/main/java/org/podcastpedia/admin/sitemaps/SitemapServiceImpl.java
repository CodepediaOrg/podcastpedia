...
public class SitemapServiceImpl implements SitemapService {
	...
	public void createSitemapForPodcastsWithFrequency(
		...
		int fileCounter = 0;
		WebSitemapGenerator wsg = WebSitemapGenerator.builder("https://www.podcastpedia.org", targetDirectory)
		...
		for(Podcast podcast : podcasts) {
			...
			if(podcast.getIdentifier()!=null){
				url= "https://www.podcastpedia.org/" + podcast.getIdentifier();
				...
				url = "https://www.podcastpedia.org" + "/podcasts/" + podcast.getPodcastId() + "/" + podcast.getTitleInUrl();
				wsmUrl = new WebSitemapUrl.Options(url)
			     							.lastMod(podcast.getPublicationDate()) // date of the last published episode
			     							.priority(0.9) //high priority just below the start page which has a default priority of 1 by default
			     							.changeFreq(changeFrequencyFromUpdateFrequency(updateFrequency))
			...
			if(nrOfURLs <= 30000){
				...
				nrOfURLs = 0;
				fileCounter++;
				wsg = WebSitemapGenerator.builder("https://www.podcastpedia.org", targetDirectory)
	...
	public void createSitemapIndexFile(String sitemapsDirectoryPath) throws MalformedURLException {
		...
		SitemapIndexGenerator sig = new SitemapIndexGenerator("https://www.podcastpedia.org", outFile);
